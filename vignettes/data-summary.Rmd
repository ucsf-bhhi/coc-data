...
```{r maps}
coc_map = read_rds("../maps/coc_display_maps.rds") %>% 
  pluck(9) %>% 
  left_join(combined_dataset, by = c("coc_number", "coc_name", "year")) %>% 
  mutate(
    tooltip = glue(
      "{coc_number} - {coc_name}: {round(homeless_per_1000_total_pop, 1)}"
    )
  ) %>%
  ggplot() +
  geom_sf_interactive(
    aes(fill = homeless_per_1000_total_pop, tooltip = tooltip), size = 0.1) +
  coord_sf(crs = 3857, datum = NULL) +
  scale_fill_distiller_interactive(palette = "GnBu", direction = 1, na.value = "grey75") +
  base_theme +
  theme(
...
girafe(ggobj = coc_map, width_svg = 10, height_svg = 9,
  options = list(
    opts_zoom(max = 5)
  )
)
