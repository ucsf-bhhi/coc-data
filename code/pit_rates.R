...
combined_data = coc_county_crosswalk %>% 
  ...
  mutate(total_pop_in_coc = total_population * pct_cnty_pop_coc / 100,
  ...
  summarise(coc_total_population = sum(total_pop_in_coc),
  ...
  mutate(homeless_rate_total_pop = overall_homeless / coc_total_population,
